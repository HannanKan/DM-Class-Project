...
for i in range(n):
    for j in range(n-i-1):
        ...
        if num%40==0:
            ...
            print(k,'Saving...')
            print('train_part...')
            df_feature.loc[train_part_index].to_csv(abs_path('train_part_x_cvr_'+str(k)+'.csv',DATA_P_DIR),index=False)
            print('test...')
            df_feature.loc[test_index].to_csv(abs_path('test_x_cvr_'+str(k)+'.csv',DATA_P_DIR),index=False)
            df_feature = pd.DataFrame()
            print('Over')
