...
train_part_y = pd.read_csv(abs_path('train_part_y.csv',DATA_P_DIR),header=None)
...
df.to_csv(abs_path('cv_onehot.csv',DATA_P_DIR),index=False)
df = []
print('Over...')
...
for feature in vector_feature:
    ...
    del data[feature]
    print(feature,int(time.time()-s),'s')
df = pd.read_csv(abs_path('cv_onehot.csv',DATA_P_DIR))
data = pd.concat([data,df],axis=1)
df = []
    ...
    for j in range(n-i-1):
        if i <= 15 and i+j+1 > 15 and i >= 1:
            ...
            if num%10==0:
                ...
                print(k,'Saving...')
                print('train_part...')
                df_feature.loc[train_part_index].to_csv(abs_path('train_part_x_CV_cvr_'+str(k)+'.csv',DATA_P_DIR),index=False)
                print('test...')
                df_feature.loc[test_index].to_csv(abs_path('test_x_CV_cvr_'+str(k)+'.csv',DATA_P_DIR),index=False)
                df_feature = pd.DataFrame()
